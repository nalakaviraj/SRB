...
var pos_local_product_list_enabled = true;
...
function pos_try_add_exact_from_suggestion(term) {
    ...
    if ($match.length) {
        ...
        if (variation_id !== null && typeof variation_id !== 'undefined') {
            ...
            pos_product_row(variation_id, null, null, 1);
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product').on('input', function() {
            ...
            if (term) {
                ...
                if (pos_local_index.loaded && pos_local_index.exact_map[key]) {
                    var item = pos_local_index.exact_map[key];
                    var is_overselling_allowed = false;
                    if ($('input#is_overselling_allowed').length) {
                        is_overselling_allowed = true;
                    }
                    ...
                    if (item.enable_stock != 1 || item.qty_available > 0 || is_overselling_allowed || for_so || is_draft) {
                        ...
                        pos_last_exact_sku = key;
                        pos_last_exact_sku_at = Date.now();
                        pos_product_row(item.variation_id, null, null, 1);
                        pos_clear_search_input();
                        return;
                    } else {
                        toastr.error(LANG.out_of_stock);
    ...
    $('#search_product').keydown(function(e) {
        ...
        if (key == 13) {
            ...
            if (input_val_enter && input_val_enter.trim() !== '') {
                var $items_enter = pos_get_suggestion_items();
                if ($items_enter.length) {
                    e.preventDefault();
        ...
        if (key == 40 || key == 38) {
            var $items_nav = pos_get_suggestion_items();
            if ($items_nav.length) {
                e.preventDefault();
                pos_set_active_suggestion(key == 40 ? 0 : ($items_nav.length - 1));
                return;
            }
        }
        if (key == 13) {
