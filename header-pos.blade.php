...
@php
    ...
    if (request()->segment(2) == 'v2') {
        $pos_redirect_url = action([\App\Http\Controllers\SellPosController::class, 'createV2']);
    } elseif (request()->segment(2) == 'v3') {
        $pos_redirect_url = action([\App\Http\Controllers\SellPosController::class, 'createV3']);
    }
@endphp
    ...
    <div
        ...
        <div class="tw-w-full md:tw-w-2/3 !tw-p-0 tw-flex tw-items-center tw-justify-between tw-gap-4 tw-flex-col md:tw-flex-row tw-hidden md:tw-flex"
            ...
            <a href="{{ action([\App\Http\Controllers\ProductController::class, 'create']) }}"
                class="tw-dw-btn tw-dw-btn-secondary tw-text-white tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 pull-right">
                Add Product
            </a>
            <a href="{{ $go_back_url }}" title="{{ __('lang_v1.go_back') }}"
                class="tw-dw-btn tw-dw-btn-primary tw-text-white tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 tw-flex tw-items-center tw-justify-center pull-right">
                <span>Office</span>
            </a>
            ...
            @can('close_cash_register')
                <button type="button" id="close_register" title="{{ __('cash_register.close_register') }}"
                    class="tw-dw-btn tw-dw-btn-error tw-text-white tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 btn-modal pull-right"
                    data-container=".close_register_modal"
                    data-href="{{ action([\App\Http\Controllers\CashRegisterController::class, 'getCloseRegister']) }}">
                    <span>{{ __('cash_register.close_register') }}</span>
                </button>
            @endcan
            ...
            @can('view_cash_register')
                <button type="button" id="register_details" title="{{ __('cash_register.register_details') }}"
                    class="tw-dw-btn tw-dw-btn-info tw-text-white tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 btn-modal pull-right"
                    ...
                    <span>{{ __('cash_register.register_details') }}</span>
                </button>
            @endcan

            <button type="button"
                class="tw-dw-btn tw-dw-btn-warning tw-text-white tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 pull-right popover-default"
                ...
                data-html="true" data-placement="bottom">
                <span>{{ __('lang_v1.sell_return') }}</span>
            </button>


            <button type="button" id="view_suspended_sales" title="{{ __('lang_v1.view_suspended_sales') }}"
                class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-neutral tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 btn-modal pull-right"
            ...
            @can('expense.add')
                <button type="button" title="{{ __('expense.add_expense') }}" data-placement="bottom"
                    class="tw-dw-btn tw-dw-btn-success tw-text-white tw-font-semibold tw-rounded-md tw-px-4 tw-h-10 btn-modal pull-right"
                    id="add_expense">
